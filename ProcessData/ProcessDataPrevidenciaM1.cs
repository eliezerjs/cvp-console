...
{
    public class ProcessDataPrevidenciaM1
    {
        private readonly IPrevidenciaM1Service _previdenciaM1Service;

        public ProcessDataPrevidenciaM1(IPrevidenciaM1Service previdenciaM1Service)
        {
            _previdenciaM1Service = previdenciaM1Service ?? throw new ArgumentNullException(nameof(previdenciaM1Service));
        }

        public async Task ProcessFilesAsync(string inputPath, string outputPath, string processedFilesPath, PrevidenciaM1Type tipo)
        {
            ...
            foreach (var file in files)
            {
                ...
                try
                {
                    ...
                    IEnumerable<byte[]> pdfsData = await _previdenciaM1Service.ConverterEGerarPrevidenciaM1PdfAsync(fileStream, tipo);
                    int pdfIndex = 1;
